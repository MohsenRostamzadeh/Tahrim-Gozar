...
public class GozarClient extends WebViewClient {
    ...
    public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
        if (Constants.useInWebView && Constants.canOverrideAllLinks)
            GozarProxySetter.getInstance(view != null ? view.getUrl() : failingUrl, view, errorCode).execute(null, true);
    ...
    public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
        if (Constants.useInWebView && Constants.canOverrideAllLinks)
            GozarProxySetter.getInstance(request != null && request.getUrl() != null ? request.getUrl().toString() : (view != null ? view.getUrl() : ""), view, error != null ? error.getErrorCode() : 0).execute(null, true);
