...
public class GozarWebView extends WebView {
    ...
    public void postUrl(@NonNull String url, @NonNull byte[] postData) {
        if (Constants.useInWebView&&Constants.canOverrideAllLinks) GozarProxySetter.getInstance(url).execute(()->super.postUrl(url, postData),false);
        else super.postUrl(url, postData);
    }
}